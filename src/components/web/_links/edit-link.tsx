...
const EditLinkForm: React.FC<EditLinkFormProps> = React.memo(({
  ...
  const paramKeys = useMemo(() => 
    ["source", "medium", "campaign", "term", "content", "referral"] as const,
    []
  );
  ...
  const isSafeToSubmit = useMemo(() =>
    ...
    isAnythingDirty,
    [isValid, urlSafetyStatus.isChecking, urlSafetyStatus.isValid, isAnythingDirty]
  );

  const currentUrl = useMemo(() => getValues("url"), [getValues]);

  // Form submission
  const onSubmit = useCallback(async (data: LinkFormValues) => {
    try {
